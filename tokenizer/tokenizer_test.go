...
func TestTokenize(t *testing.T) {
	...
	wdict := map[string]*dicttypes.Word{
		...
		hiraganaA.Simplified:  &hiraganaA,
		hiraganaK.Simplified:  &hiraganaK,
	}
	q := TextToken{
		Token:     "全",
		DictEntry: quan,
	...
	hiraganaAToken := TextToken{
		Token:     "あ",
		DictEntry: hiraganaA,
		Senses:    []dicttypes.WordSense{},
	}
	hiraganaKaToken := TextToken{
		Token:     "か",
		DictEntry: hiraganaK,
		Senses:    []dicttypes.WordSense{},
	}
	...
	testCases := []struct {
		...
		{
			name: "One Hiragana token",
			in:   "あ",
			want: []TextToken{hiraganaAToken},
		},
		{
			name: "Two Hiragana tokens",
			in:   "あか",
			want: []TextToken{hiraganaAToken, hiraganaKaToken},
		},
	}
	for _, tc := range testCases {
